...
| `stream_context_create(): int` | `stream_context_create(array $options = [], array $params = []): resource\|false` | 占位实现：返回 `0`；TinyPHP 的 `stream_socket_server`/`stream_socket_client` 等忽略 context 参数 `(void)context;`，故本函数仅为 API 兼容存在，不接收 options/params 参数 |
