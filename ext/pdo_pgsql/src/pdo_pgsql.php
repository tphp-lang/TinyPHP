...
//   2. 指针以 int 存储（t_int 64位），直接传递给 C 函数（不用 c_int 截断）
...
{
    return php_int(_pgpdo_get_pid($dbh));
}
...
{
    $arr = _pgpdo_get_notify($dbh, $result_type, $timeout_ms);
    if ($arr == 0) {
        return [];
    }
    return $arr;
...
{
    return php_int(_pgpdo_pgconn($dbh));
}
