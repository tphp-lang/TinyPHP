...
//   - 方法内部通过 pdo_driver_* 包装函数调用，driver 内部分发到 sqlite3_*()
...
class PDOStatement
{
    ...
    public int $db = 0;
    // 驱动指针（pdo_driver_t*，从 PDO 继承，所有调用通过 driver 分发）
    public int $driver = 0;
    ...
    public function __construct(int $driver, int $db, int $stmt, string $sql)
    {
        $this->driver = $driver;
        $this->db = $db;
        $this->stmt = $stmt;
    ...
    {
        if ($this->stmt != 0) {
            pdo_driver_finalize($this->driver, $this->stmt);
            $this->stmt = 0;
        }
    }

    // ── _throwDbError: 通过 driver 接口获取 db 错误消息并抛出异常 ──
    private function _throwDbError(string $prefix): void
    {
        $emsg = php_str(pdo_driver_errmsg($this->driver, $this->db));
        pdo_throw_msg($prefix . ": " . $emsg);
    }

    // ── _throwStmtError: 通过 driver 接口获取 stmt 错误消息并抛出异常 ──
    //   MySQL driver 的 errmsg 基于 conn，stmt 借用 conn 的错误
    private function _throwStmtError(string $prefix): void
    {
        $emsg = php_str(pdo_driver_errmsg($this->driver, $this->db));
        pdo_throw_msg($prefix . ": " . $emsg);
    }
    ...
    public function bindValueInt(int $param, int $value, int $type = 2): bool
    {
        ...
        $this->_autoResetForBind();
        $idx = $param;
        $rc = 0;
        if ($type == 0) {
            // PARAM_NULL
            $rc = php_int(pdo_driver_bind_null($this->driver, $this->stmt, c_int($idx)));
        } elseif ($type == 1 || $type == 5) {
            // PARAM_INT / PARAM_BOOL
            $rc = php_int(pdo_driver_bind_int($this->driver, $this->stmt, c_int($idx), c_int($value)));
        } else {
            // PARAM_STR / PARAM_LOB — int 转字符串后绑定
            $str = c_str(strval($value));
            $len = php_int(pdo_str_len($str));
            $rc = php_int(pdo_driver_bind_text($this->driver, $this->stmt, c_int($idx), $str, c_int($len)));
        }
        if ($rc != 0) {
            $this->_throwStmtError("PDOStatement::bindValueInt: bind failed");
            return false;
        }
        return true;
    }
    ...
    public function bindValueStr(int $param, string $value, int $type = 2): bool
    {
        ...
        $rc = php_int(pdo_driver_bind_text($this->driver, $this->stmt, c_int($idx), $str, c_int($len)));
        if ($rc != 0) {
            $this->_throwStmtError("PDOStatement::bindValueStr: bind failed");
            return false;
        }
        return true;
    }
    ...
    public function bindValueNamedInt(string $param, int $value, int $type = 2): bool
    {
        ...
        $this->_autoResetForBind();
        $name = c_str($param);
        $idx = php_int(pdo_driver_bind_param_index($this->driver, $this->stmt, $name));
        if ($idx == 0) {
            $this->_throwStmtError("PDOStatement::bindValueNamedInt: unknown parameter name");
            return false;
        }
        $rc = 0;
        if ($type == 0) {
        ...
        if ($rc != 0) {
            $this->_throwStmtError("PDOStatement::bindValueNamedInt: bind failed");
            return false;
        }
        return true;
    }
    ...
    public function bindValueNamedStr(string $param, string $value, int $type = 2): bool
    {
        ...
        $idx = php_int(pdo_driver_bind_param_index($this->driver, $this->stmt, $name));
        if ($idx == 0) {
            $this->_throwStmtError("PDOStatement::bindValueNamedStr: unknown parameter name");
            return false;
        }
        $str = c_str($value);
        $len = php_int(pdo_str_len($str));
        $rc = php_int(pdo_driver_bind_text($this->driver, $this->stmt, c_int($idx), $str, c_int($len)));
        if ($rc != 0) {
            $this->_throwStmtError("PDOStatement::bindValueNamedStr: bind failed");
            return false;
        }
        return true;
    }

    // ── _autoResetForBind: 若语句已 step 过（done 或 firstFetch），自动 reset 以允许重新 bind ──
    //   SQLite 在语句处于 ROW/DONE 状态时调用 sqlite3_bind_* 会返回 SQLITE_MISUSE，
    //   PHP 原生 PDO 在 execute() 内部 reset，但 TinyPHP 的 bind 直接调 sqlite3_bind_*，
    //   因此需在 bind 前 auto-reset 以支持 "prepare → bind → execute → bind → execute" 循环
    private function _autoResetForBind(): void
    {
        if ($this->done || $this->firstFetch) {
            pdo_driver_reset($this->driver, $this->stmt);
            $this->done = false;
    ...
    public function execute(array $params = []): bool
    {
        ...
        if ($this->executed) {
            pdo_driver_reset($this->driver, $this->stmt);
            pdo_driver_clear_bindings($this->driver, $this->stmt);
        }
        $this->executed = true;
        $this->done = false;
        $this->firstFetch = false;

        // 绑定传入的参数数组（driver 内部处理 t_var 类型分发，避免 mixed 暴露到 PHP API）
        if (count($params) > 0) {
            pdo_driver_bind_params($this->driver, $this->stmt, $params);
        }

        // 执行第一步（预取一行，判断是否有结果集）
        $rc = php_int(pdo_driver_step($this->driver, $this->stmt));
        if ($rc == 100) {
            // SQLITE_ROW — 有数据，预取了一行
            $this->firstFetch = true;
            $this->columnCount = php_int(pdo_driver_data_count($this->driver, $this->stmt));
            if ($this->columnCount == 0) {
                $this->columnCount = php_int(pdo_driver_column_count($this->driver, $this->stmt));
            }
        } elseif ($rc == 101) {
            // SQLITE_DONE — 无数据（INSERT/UPDATE/DELETE 或空结果集）
            $this->done = true;
            $this->columnCount = php_int(pdo_driver_column_count($this->driver, $this->stmt));
            $this->rowCount = php_int(pdo_driver_changes($this->driver, $this->db));
        } else {
            // 错误
            $this->_throwStmtError("PDOStatement::execute: step failed");
            return false;
        }
        return true;
    }
    ...
    public function fetch(int $mode = 0): array
    {
        ...
        if ($this->firstFetch) {
            ...
            if ($rc == 101) {
                ...
                $this->_throwStmtError("PDOStatement::fetch: step failed");
                return [];
            }
        }
        ...
        $cc = $this->columnCount;
        if ($cc == 0) {
            $cc = php_int(pdo_driver_column_count($this->driver, $this->stmt));
            $this->columnCount = $cc;
        }
        $arr = [];
        if ($mode == 3) {
            ...
            // FETCH_ASSOC: 关联数组
            for ($i = 0; $i < $cc; $i = $i + 1) {
                $name = php_str(pdo_driver_column_name($this->driver, $this->stmt, c_int($i)));
    ...
    {
        $c = c_int($col);
        $t = php_int(pdo_driver_column_type($this->driver, $this->stmt, $c));
        if ($t == 5) {
            // SQLITE_NULL → 空字符串
            return "";
        } elseif ($t == 1) {
            // SQLITE_INTEGER → 转字符串
            $v = php_int(pdo_driver_column_int64($this->driver, $this->stmt, $c));
            return strval($v);
        } elseif ($t == 2) {
            // SQLITE_FLOAT → 转字符串（用独立变量避免与 int $v 类型冲突）
            $fv = pdo_driver_column_double($this->driver, $this->stmt, $c);
            return strval($fv);
        } else {
            // SQLITE_TEXT(3) / SQLITE_BLOB(4) — 统一按字符串返回
            // pdo_driver_column_text 返回 SQLite 借用指针（下次 step/reset 后失效），
            // pdo_str_from_ptr 内部深拷贝到 str_pool，无泄漏
            $len = php_int(pdo_driver_column_bytes($this->driver, $this->stmt, $c));
            return pdo_str_from_ptr(pdo_driver_column_text($this->driver, $this->stmt, $c), c_int($len));
        }
    }

    // ── _fetchColumnInt: 读取当前行指定列的整数值（内部辅助）──
    //   null/float 列返回 0；TEXT 列尝试解析为 int（MySQL 文本协议所有列都是字符串）
    private function _fetchColumnInt(int $col): int
    {
        ...
        if ($t == 1) {
            // SQLITE_INTEGER
            return php_int(pdo_driver_column_int64($this->driver, $this->stmt, $c));
        } elseif ($t == 3 || $t == 4) {
            // TEXT/BLOB — MySQL 文本协议所有列都是字符串，尝试解析为 int
            $len = php_int(pdo_driver_column_bytes($this->driver, $this->stmt, $c));
            $sv = pdo_str_from_ptr(pdo_driver_column_text($this->driver, $this->stmt, $c), c_int($len));
            // 字符串到 int 的转换（AOT 类型转换）
            $iv = intval($sv);
            return $iv;
        }
        return 0;
    ...
    public function fetchColumnInt(int $col = 0): int
    {
        ...
        if ($this->firstFetch) {
            ...
            if ($rc == 101) {
                ...
                $this->_throwStmtError("PDOStatement::fetchColumnInt: step failed");
                return 0;
            }
        }
    ...
    public function columnCount(): int
    {
        ...
        return php_int(pdo_driver_column_count($this->driver, $this->stmt));
    }
    ...
    {
        $rc = php_int(pdo_driver_errcode($this->driver, $this->db));
    ...
    public function errorInfo(): array
    {
        ...
        $msg = php_str(pdo_driver_errmsg($this->driver, $this->db));
    ...
    {
        $c = c_int($col);
        $name = php_str(pdo_driver_column_name($this->driver, $this->stmt, $c));
        $declType = php_str(pdo_driver_column_decltype($this->driver, $this->stmt, $c));
        $t = php_int(pdo_driver_column_type($this->driver, $this->stmt, $c));
...
class PDO
{
    ...
    public int $db = 0;
    // 驱动指针（pdo_driver_t*，所有调用通过 driver 分发）
    public int $driver = 0;
    ...
    public function __construct(string $dsn, string $username = "", string $password = "", array $options = [])
    {
        ...
        $colon = strpos($dsn, ':');
        if ($colon === false) {
            pdo_throw_msg("PDO::__construct: DSN must contain ':'");
            return;
        }
        $prefix = substr($dsn, 0, $colon);
        $drv = php_int(pdo_driver_find($prefix));
        if ($drv == 0) {
            pdo_throw_msg("PDO: driver '{$prefix}' not available");
            return;
        }
        $this->driver = $drv;
        // 通过 driver 打开数据库（pdo_driver_open 接收 t_string，直接传 PHP 字符串）
        $db = php_int(pdo_driver_open($this->driver, $dsn, c_int($flags), $username, $password));
        if ($db == 0) {
            // open 失败：通过 pdo_driver_last_open_error 获取详细错误信息
            $detail = php_str(pdo_driver_last_open_error());
            $dlen = strlen($detail);
            if ($dlen > 0) {
                pdo_throw_msg("PDO::__construct: " . $detail);
            } else {
                pdo_throw_msg("PDO::__construct: failed to open database");
            }
            return;
        }
        $this->db = $db;
        // 设置 busy timeout
        pdo_driver_busy_timeout($this->driver, $this->db, c_int($timeout * 1000));
        // 启用扩展结果码（更详细的错误信息）
        pdo_driver_extended_result_codes($this->driver, $this->db, c_int(1));
    }

    public function __destruct()
    {
        if ($this->db != 0) {
            pdo_driver_close($this->driver, $this->db);
            $this->db = 0;
        }
    }
    ...
    public function prepare(string $query, array $options = []): PDOStatement|Exception
    {
        ...
        $sqlC = c_str($query);
        $stmt = php_int(pdo_driver_prepare($this->driver, $this->db, $sqlC));
        if ($stmt == 0) {
            $this->_throwDbError("PDO::prepare: failed to prepare statement");
        }
        $st = new PDOStatement($this->driver, $this->db, $stmt, $query);
    ...
    public function exec(string $statement): int|Exception
    {
        ...
        $n = php_int(pdo_driver_exec($this->driver, $this->db, $sqlC));
        if ($n < 0) {
            $this->_throwDbError("PDO::exec");
        }
        return $n;
    }
    ...
    public function quote(string $string, int $type = 2): string|Exception
    {
        ...
        $s = c_str($string);
        return pdo_driver_quote($this->driver, $s);
    }
    ...
    public function lastInsertId(string $name = ""): string|Exception
    {
        ...
        $id = php_int(pdo_driver_last_insert_rowid($this->driver, $this->db));
        // PHP 原生返回字符串（避免大整数溢出）
        return strval($id);
    }
    ...
    public function beginTransaction(): bool
    {
        ...
        $n = php_int(pdo_driver_exec($this->driver, $this->db, c_str($sql)));
        if ($n < 0) {
            return false;
        }
    ...
    public function commit(): bool
    {
        ...
        $n = php_int(pdo_driver_exec($this->driver, $this->db, c_str("COMMIT")));
        if ($n < 0) {
            return false;
        }
    ...
    public function rollBack(): bool
    {
        ...
        $n = php_int(pdo_driver_exec($this->driver, $this->db, c_str("ROLLBACK")));
        if ($n < 0) {
            return false;
        }
        $this->inTransaction = false;
    ...
    public function getAttributeStr(int $attribute): string
    {
        ...
        if ($a == 16) {
            // ATTR_DRIVER_NAME
            return php_str(pdo_driver_name($this->driver));
        } elseif ($a == 4 || $a == 5 || $a == 6) {
            // ATTR_SERVER_VERSION / ATTR_CLIENT_VERSION / ATTR_SERVER_INFO
            return php_str(pdo_driver_server_version($this->driver, $this->db));
        }
        return "";
    }
    ...
    public function setAttribute(int $attribute, int $value): bool
    {
        ...
        if ($a == 3) {
            ...
            if ($this->db != 0) {
                $ms = $value * 1000;
                pdo_driver_busy_timeout($this->driver, $this->db, c_int($ms));
            }
            return true;
        } elseif ($a == 1000) {
