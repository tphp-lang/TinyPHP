...
## 8. 字符串操作基准 (bench_string, TCC)

测试环境: Windows x64, TinyPHP + TCC, 2026-07-21  
覆盖 SSO/ROPE/Arena 优化路径，N=100K, M=N/10

### 原始数据 (ns, 越低越好)

| # | 测试项 (iter) | TCC | ns/op |
|---|--------------|-----|-------|
| 1 | concat 2 frags ×100K | 6,581,100 | 65.8 |
| 2 | concat 4 frags ×100K | 13,046,100 | 130.5 |
| 3 | concat vars ×100K | 25,210,300 | 252.1 |
| 4 | concat loop 20 ×10K | 44,410,300 | 4441.0 |
| 5 | substr ×100K | 6,761,800 | 67.6 |
| 6 | strpos ×100K | 11,263,700 | 112.6 |
| 7 | str_replace ×100K | 57,871,200 | 578.7 |
| 8 | strtolower ×100K | 13,532,800 | 135.3 |
| 9 | trim ×100K | 7,553,100 | 75.5 |
| 10 | strlen ×100K | 726,800 | 7.3 |
| 11 | explode+implode ×10K | 24,136,800 | 2413.7 |
| 12 | sprintf ×10K | 11,712,900 | 1171.3 |

### 关键观察

- **strlen** 仅 7.3 ns/op：SSO 内联存储长度，零 strlen 调用
- **concat 2 frags** 65.8 ns/op：SSO 24B 内联，零堆分配
- **concat 4 frags** 走 ROPE 多片段拼接路径，避免 O(n²) 复制
- **str_replace** 578.7 ns/op：相对较慢，因涉及子串搜索 + 多次拼接
- **explode+implode** 2413.7 ns/op：Arena Allocator + 精确容量预分配

---

## 9. 哈希函数基准 (bench_hash, TCC)

测试环境: Windows x64, TinyPHP + TCC, 2026-07-21  
N=100K, 数据规模 11B / 1KB / 100KB

### 原始数据 (ns, 越低越好)

| # | 测试项 (iter) | TCC | ns/op | 吞吐 (100KB) |
|---|--------------|-----|-------|--------------|
| 1 | md5(11B) ×100K | 62,269,500 | 622.7 | — |
| 2 | md5(1KB) ×10K | 68,178,500 | 6817.9 | 146 MB/s |
| 3 | md5(100KB) ×1K | 639,463,300 | 639463.3 | 156 MB/s |
| 4 | sha1(11B) ×100K | 103,719,600 | 1037.2 | — |
| 5 | sha1(1KB) ×10K | 125,452,000 | 12545.2 | 80 MB/s |
| 6 | sha1(100KB) ×1K | 1,200,534,100 | 1200534.1 | 83 MB/s |
| 7 | sha256(11B) ×100K | 139,782,700 | 1397.8 | — |
| 8 | sha256(1KB) ×10K | 181,186,700 | 18118.7 | 55 MB/s |
| 9 | sha256(100KB) ×1K | 1,737,690,200 | 1737690.2 | 57 MB/s |
| 10 | crc32(11B) ×100K | 4,425,300 | 44.3 | — |
| 11 | hmac-sha256 ×10K | 57,091,400 | 5709.1 | — |
| 12 | sha512(11B) ×100K | 213,561,700 | 2135.6 | — |

### 关键观察

- **crc32** 44.3 ns/op：最快的哈希，仅 32 位运算
- **md5** 吞吐 ~156 MB/s：4 轮/块，最快的安全哈希
- **sha1** 吞吐 ~83 MB/s：5 轮/块，比 md5 慢约 2x
- **sha256** 吞吐 ~57 MB/s：64 轮/块，比 sha1 慢约 1.5x
- **sha512** 比 sha256 慢：尽管块更大，但 64 位运算在 TCC 下未充分优化
- **hmac-sha256** 5709 ns/op：约 sha256 单次 4x 开销（IPAD/OPAD 两轮 + 内部 sha256）

---

## 10. SQLite PDO 基准 (bench_sqlite, TCC)

测试环境: Windows x64, TinyPHP + TCC, 2026-07-21  
N=10K, SQLite `:memory:` 数据库

### 原始数据 (ns, 越低越好)

| # | 测试项 (iter) | TCC | ns/op | 备注 |
|---|--------------|-----|-------|------|
| 1 | connect+create table | 914,200 | — | 单次开销 |
| 2 | insert (no txn) ×1K | 8,494,100 | 8494.1 | 无事务，每条独立提交 |
| 3 | insert (txn) ×10K | 52,663,100 | 5266.3 | 事务批量，比无事务快 1.6x |
| 4 | insert (prepare) ×10K | 22,181,900 | 2218.2 | 预处理 + 事务，最快路径 |
| 5 | select all (fetchAll) | 10,796,600 | — | 10K 行全表扫描 |
| 6 | select by id ×1K | 11,832,000 | 11832.0 | 每次新建 stmt |
| 7 | select (prepare) ×1K | 2,605,800 | 2605.8 | 复用 stmt，比 #6 快 4.5x |
| 8 | update ×1K | 9,644,700 | 9644.7 | 事务批量 UPDATE |
| 9 | count(*) ×1K | 10,514,500 | 10514.5 | 聚合查询 |
| 10 | quote ×10K | 5,947,800 | 594.8 | 字符串转义 |

### 关键观察

- **预处理 INSERT** 2218 ns/op：事务 + 预处理是最优写入路径，比无事务快 3.8x
- **事务批量 INSERT** 5266 ns/op：比无事务快 1.6x，事务开销分摊到 10K 条
- **预处理 SELECT** 2606 ns/op：stmt 复用比每次 query 快 4.5x（省去 parse/plan）
- **insert (prepare) vs insert (txn)**：预处理节省 SQL 解析开销，2218 vs 5266 ns/op = 2.4x 提升
- **fetch all 10K 行** 10.8ms：sqlite3_step 循环 + Arena 分配，无额外拷贝
- **quote** 594.8 ns/op：单次转义 + SSO 拷贝
...
| ext/zlib (内置 zlib 1.3.2 静态编译) | 29 | ✅ |
| ext/zip (复用内置 zlib) | 18 | ✅ |
| ext/exif (纯 phpc 实现) | 5 | ✅ |
