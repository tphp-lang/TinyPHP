...
| 16 | [libevent](#16-libevent)     | ⭐⭐⭐⭐  | libevent       | 12  |
...
| 库                              | 说明                | 链接                                          |
| ------------------------------ | ----------------- | ------------------------------------------- |
| **libevent**                   | 高性能事件通知库 (BSD 协议) | libevent.org / github.com/libevent/libevent |
| **PHP 源码** **`ext/libevent/`** | OO 接口参考           | `libevent.c` (\~8KB)                        |
| **libev**                      | 轻量替代 (单文件，API 类似) | software.schmorp.de/pkg/libev.html          |
| **libuv**                      | Node.js 使用的跨平台事件库 | libuv.org                                   |
...
// 事件标志常量 ( bitmask )
// ================================================================
const EV_TIMEOUT = 0x01;   // 超时事件
const EV_READ = 0x02;      // 可读事件
const EV_WRITE = 0x04;     // 可写事件
const EV_SIGNAL = 0x08;    // 信号事件
const EV_PERSIST = 0x10;   // 持续事件 (不自动删除)
const EV_ET = 0x20;        // 边缘触发 (epoll/kqueue)

// ================================================================
// 循环标志
// ================================================================
const EVLOOP_ONCE = 0x01;      // 只运行一次
const EVLOOP_NONBLOCK = 0x02;  // 非阻塞模式
...
class EventBase {
     ...
     * 创建事件基础对象
     */
    public static function new(): self|false;

    /**
     * 运行事件循环
     *
     * @param int $flags 循环标志: 0=默认, EVLOOP_ONCE=只运行一次
     ...
     * 中断事件循环
     */
    public function loopbreak(): bool;

    /**
     * 延迟退出事件循环
     *
     * @param float $timeout 延迟秒数，0=立即退出
     */
    public function loopexit(float $timeout = 0): bool;
}
...
class Event {
     ...
     * 创建事件对象
     *
     * @param EventBase $base 事件基础
     * @param int|resource $fd 文件描述符 (int) 或 socket 资源，信号事件传信号值
     * @param int $what 事件标志组合: EV_READ|EV_WRITE|EV_SIGNAL|EV_PERSIST
     * @param callable $callback 回调函数: function(resource $fd, int $what, mixed $arg): void
     * @param mixed $arg 传递给回调的用户数据
     */
    public static function new(EventBase $base, int|resource $fd, int $what,
                               callable $callback, mixed $arg = null): self|false;

    /**
     * 将事件添加到事件基础，开始监听
     *
     * @param float $timeout 超时秒数，-1 或 0 表示无超时
     */
...
class EventBuffer {
     ...
     *
     * @param EventBase $base 事件基础
     * @param resource $fd socket 资源
     * @param int $what 事件标志: EV_READ|EV_WRITE
     * @param callable $callback 回调函数
     * @param mixed $arg 传递给回调的用户数据
     */
    public static function new(EventBase $base, resource $fd, int $what,
                               callable $callback, mixed $arg = null): self|false;
...
class Main {
    public function main(): void {
        // 1. 基本定时器
        $base = EventBase::new();
        $timer = Event::new($base, -1, EV_TIMEOUT | EV_PERSIST, function($fd, $what, $arg) {
            echo "Timer fired: $arg\n";
        }, "hello");
        $timer->add(1.0);  // 每秒触发
        $base->loop();
        ...
        $readEvent = Event::new($base, $server, EV_READ | EV_PERSIST, function($fd, $what) {
        ...
        // 3. 信号处理
        $base = EventBase::new();
        $sig = Event::new($base, SIGINT, EV_SIGNAL | EV_PERSIST, function() use ($base) {
...
static void event_callback(evutil_socket_t fd, short what, void *arg) {
    ...
    // 调用 PHP 回调: cb->func(fd, what, cb->env)
}

// 创建事件
void* tphp_fn_event_new(void *base, t_int fd, t_int what,
                         t_callback callback, t_var arg) {
    struct event *ev = event_new(
        (struct event_base*)base,
        fd,
        what,
        event_callback,
        tphp_fn_event_new_env(callback, arg)  // 包装环境
    );
    return ev;
}

// 事件循环
t_int tphp_fn_event_base_loop(void *base, t_int flags) {
...
| 16 | libevent       | \~400  | ⭐⭐⭐⭐       | libevent       | 12  | 2-3 天 |

---

## 17. yield/Generator (协程)

### 状态：待实现（依赖 libuv + libco）

### 说明

PHP 的 yield/Generator 基于纤程（Fiber）实现，纤程提供用户态的上下文切换功能。

**TinyPHP 当前状态**：
-纤程实现存在 Windows 兼容性问题（原生 Fiber API 不稳定）
- 后续计划集成 libuv 事件循环时，同时引入 libco 提供纤程支持

### 实现依赖

| 依赖 | 说明 | 用途 |
|------|------|------|
| libuv | 跨平台事件循环 | 异步 I/O、定时器、信号 |
| libco | 微信团队的协程库 | 纤程/协程上下文切换 |

### 为什么选择 libco

| 特性 | libco | 原因 |
|------|-------|------|
| 跨平台 | Windows/Linux/macOS | 与 TinyPHP 目标一致 |
| 基于 ucontext | 纯 C 实现 | 无需汇编代码 |
| 性能 | ~50ns/切换 | 满足需求 |
| 稳定性 | 微信生产环境验证 | 可靠 |
| 依赖 | 无外部依赖 | 符合 AOT 设计 |
...
```c
// 创建生成器
Generator *g = Generator_new(range_gen);

// 遍历
while (Generator_valid(g)) {
    t_int val = Generator_current(g);
    Generator_next(g);
}
...
1. **集成 libuv**：事件循环、异步 I/O
2. **集成 libco**：纤程/协程上下文切换
3. **实现 yield.h**：PHP Generator 语法映射
4. **Parser 修改**：支持 yield 关键字
5. **CodeGenerator 修改**：生成协程相关代码

### 性能预期

| 操作 | 预期开销 | 说明 |
|------|----------|------|
| yield 暂停 | ~50ns | libco 上下文切换 |
| next 恢复 | ~50ns | libco 上下文切换 |
| 创建 Generator | ~1μs | 分配栈 + 初始化 |

### 参考

- PHP Fiber: `Zend/zend_fibers.c`
- PHP Generator: `Zend/zend_generators.c`
- libco: github.com/Tencent/libco
- libuv: libuv.org
