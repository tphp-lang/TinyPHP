...
trait ArrayOps
{
    ...
    private function printStringRange(StringRangeNode $e): void
    {
        ...
        $this->b->movRR(X64Builder::RSI, X64Builder::RAX);  // RSI = ptr (syscall needs buf in RSI)
        // RDX already has len
    ...
    private function genArrayIndexRead(array $info, ExprNode $index): void
    {
        ...
        // Evaluate index → RAX
        $this->genExpr($index);
        // Bounds check: if index >= len, clamp to 0
        ...
        // Compute address: rbp + offset + 16 + index * elemSize
        ...
        $this->b->addRR(X64Builder::RAX, X64Builder::RBP);
        ...
        match ($elemType) {
            ...
            default          => $this->b->movRM64(X64Builder::RAX, X64Builder::RAX, 0),
        };
    }
    ...
    private function printArray(ExprNode $e): void
    {
        ...
        $this->b->emit("\x48\x8D\x35"); $this->b->rel32($prefixLab);
        ...
        $this->b->emit("\x45\x31\xE4");                          // xor r12d, r12d
        ...
        $this->b->emit("\x48\x8D\x35"); $this->b->rel32($sep);
        $this->b->movRI32(X64Builder::RDX, 2);
        $this->doWrite(X64Builder::RSI, X64Builder::RDX);
        ...
        $this->b->emit("\x48\x8D\x35"); $this->b->rel32($endLab);
    ...
    {
        $this->b->movRM64(X64Builder::RSI, $addrReg, 0);       // rsi = ptr
        $this->b->movRM64(X64Builder::RDX, $addrReg, 8);       // rdx = len
        $this->doWrite(X64Builder::RSI, X64Builder::RDX);
    }

    private function echoArrayBoolElem(int $addrReg): void
    {
        $this->b->movRM64(X64Builder::RAX, $addrReg, 0);
    ...
    private function printCallableString(): void
    {
        ...
        $this->b->movRI32(X64Builder::RDX, 10);
        $this->doWrite(X64Builder::RSI, X64Builder::RDX);
    }
    ...
    /**
     * Emit arg setup for closure calls (Linux / System V AMD64).
    ...
    private function emitClosureArgs(array $args): void
    {
        ...
        foreach ($args as $arg) {
            ...
            if ($argType === TphpType::String) {
                ...
                // Save ptr first - RAX may be overwritten when saving RDX
                if ($paramRegs[$ri] !== X64Builder::RAX) {
                    $this->b->movRR($paramRegs[$ri], X64Builder::RAX);
                }
                // Save len - must be done before next genExpr() which overwrites RDX
